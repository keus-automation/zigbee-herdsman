...
class Device extends Entity {
  ...
  get keusDevice(): boolean {
    return this._keusDevice;
  }
  set keusDevice(keusDevice) {
    this._keusDevice = keusDevice;
  }
  get isDeviceVirtual(): boolean {
    return this._isDeviceVirtual;
  }
  set isDeviceVirtual(isDeviceVirtual) {
  ...
  private static devices: {
    [dbInstKey: string]: { [ieeeAddr: string]: Device };
  } = null;
  ...
  private static fromDatabaseEntry(
    ...
    return new Device(
      ...
      entry.lastSeen || null,
      dbInstKey,
      entry.keusDevice || true,
      entry.isDeviceVirtual || true,
    );
  }
  ...
  private toDatabaseEntry(): DatabaseEntry {
    ...
    return {
      ...
      isDeviceVirtual: this._isDeviceVirtual,
    };
  }
  ...
  private static loadFromDatabaseIfNecessary(dbInstKey: string): void {
    if (!Device.devices[dbInstKey]) {
      ...
      const entries = Entity.databases[dbInstKey].getEntries([
        "Coordinator",
        "EndDevice",
        "Router",
        "GreenPower",
  ...
  public static byNetworkAddress(
    ...
    Device.loadFromDatabaseIfNecessary(dbInstKey);
    return Object.values(Device.devices[dbInstKey]).find(
  ...
  public static byType(dbInstKey: string, type: DeviceType): Device[] {
    Device.loadFromDatabaseIfNecessary(dbInstKey);
    return Object.values(Device.devices[dbInstKey]).filter(
      (d) => d.type === type
    );
  }
  ...
  public static create(
    ...
    dbInstKey: string,
    keusDevice: boolean = false,
    IsDeviceVirtual: boolean = false
  ): Device {
    Device.loadFromDatabaseIfNecessary(dbInstKey);
  ...
  private async interviewInternal(): Promise<void> {
    const nodeDescriptorQuery = async (): Promise<void> => {
      const nodeDescriptor = await Entity.adapters[
        this._dbInstKey
    ...
    if (keusEndPoint) {
      try {
        let simpleDescriptor = await Entity.adapters[
          this._dbInstKey
        ].simpleDescriptor(this.networkAddress, keusEndPoint);
        ...
        debug.log(
          `Keus Interview - got simple descriptor for endpoint '${endpoint.ID}' device '${this.ieeeAddr}'`
        );
        ...
        if (keusEndPoint === 1) {
          ...
          const stateBefore = await endpoint.read("ssIasZone", [
            "iasCieAddr",
            "zoneState",
          ]);
          debug.log(
          ...
          if (
            ...
            for (let attempt = 0; attempt < 20; attempt++) {
              await Wait(500);
              const stateAfter = await endpoint.read("ssIasZone", [
                "iasCieAddr",
                "zoneState",
              ]);
              debug.log(
        ...
        debug.log(
          `Error with Keus device pairing, Simple Descriptor Request Failed`
        );

        throw new Error(
          "Keus Interview - Simple Descriptor Failed Or IAS Failed"
        );
      }
    } else {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        activeEndpoints = await Entity.adapters[
          this._dbInstKey
        ].activeEndpoints(this.networkAddress);
        break;
      } catch (error) {
        debug.log(
    ...
    this._endpoints = activeEndpoints.endpoints
      .filter((e) => e !== 0)
      .map((e): Endpoint => {
        return Endpoint.create(
          e,
    ...
    for (const endpoint of this.endpoints) {
      const simpleDescriptor = await Entity.adapters[
        this._dbInstKey
      ].simpleDescriptor(this.networkAddress, endpoint.ID);
  ...
  public async removeFromNetwork(): Promise<void> {
    await Entity.adapters[this._dbInstKey].removeDevice(
      this.networkAddress,
      this.ieeeAddr
    );
